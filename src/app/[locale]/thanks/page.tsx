...
export default async function ThanksPage({ params }: Props) {
  ...
  await getTranslations("thanks");

  return (
    <main className="min-h-screen bg-gradient-to-br from-brand-50 via-white to-neutral-100 px-4 py-16 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-3xl flex-col items-center rounded-3xl border border-brand-100 bg-white/95 p-8 text-center shadow-smooth sm:p-12">
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
          <Link
            href={`/${locale}`}
            className="rounded-full bg-brand-200 px-6 py-3 text-sm font-semibold text-white transition hover:bg-brand-500"
          >
            Kembali ke beranda
          </Link>
          <a
